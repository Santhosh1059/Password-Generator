...
(function() {
  ...
  function generate() {
    ...
    if (!pool.length) {
      output.textContent = "Select at least one charset";
      return "";
    }
    let pw = "";
    for (let i = 0; i < len; i++) {
  ...
  async function deriveKey(pass, salt, iterations = 12e4) {
  ...
  async function decryptJSON(payload, pass) {
    if (typeof payload === "string") {
      ...
      const ct2 = raw.slice(12);
      const key2 = await deriveKey(pass, null);
      const dec2 = await crypto.subtle.decrypt({ name: "AES-GCM", iv: iv2 }, key2, ct2);
      return JSON.parse(new TextDecoder().decode(dec2));
    }
    const salt = Uint8Array.from(atob(payload.salt), (c) => c.charCodeAt(0));
    const iv = Uint8Array.from(atob(payload.iv), (c) => c.charCodeAt(0));
    const ct = Uint8Array.from(atob(payload.cipher), (c) => c.charCodeAt(0));
    const key = await deriveKey(pass, salt, payload.iterations || 12e4);
  ...
  function saveNote() {
    ...
    if (!pw || pw.startsWith("Select"))
      return alert("Generate a password first");
    ...
    notesPromise.then(async (notes) => {
      ...
      if (sessionPass) {
        try {
          ...
          console.warn("Encryption save failed", e);
  ...
  function renderNotes() {
    ...
    Promise.resolve(res).then((notes) => {
      ...
      if (!notes || !notes.length) {
        notesList.innerHTML = '<li class="smallText">No saved passwords yet</li>';
        return;
      }
      for (const n of notes) {
        ...
        sub.textContent = n.pw;
        sub.className = "smallText";
        ...
        copy.onclick = () => {
          navigator.clipboard.writeText(n.pw);
        };
  ...
  function renderHistory() {
    ...
    const minLen = Number(histMinLen.value) || 0;
    const minStr = Number(histMinStr.value) || 0;
    historyList.innerHTML = "";
    const filtered = h.filter((r) => {
      if (filter && !(r.name.toLowerCase().includes(filter) || r.pw.toLowerCase().includes(filter)))
        return false;
      if (minLen && r.pw.length < minLen)
        return false;
      if (minStr && r.strength < minStr)
        return false;
      return true;
    });
    if (!filtered.length) {
      historyList.innerHTML = '<li class="smallText">No history matches</li>';
      return;
    }
    for (const r of filtered) {
      ...
      meta.className = "meta";
      const title = document.createElement("div");
      title.textContent = `${r.name} \u2014 ${new Date(r.ts).toLocaleString()}`;
      title.style.fontWeight = "700";
      const sub = document.createElement("div");
      sub.textContent = `${r.pw} \xB7 ${r.strength}`;
      sub.className = "smallText";
      meta.appendChild(title);
      meta.appendChild(sub);
      const actions = document.createElement("div");
      ...
      copy.textContent = "Copy";
      copy.onclick = () => {
        navigator.clipboard.writeText(r.pw);
      };
      actions.appendChild(copy);
      li.appendChild(meta);
      li.appendChild(actions);
  ...
  async function deleteNote(id) {
    ...
    if (sessionPass) {
      try {
        const payload = await encryptJSON(filtered, sessionPass);
        localStorage.setItem("pw_notes_enc", JSON.stringify(payload));
        localStorage.removeItem("pw_notes");
      } catch (e) {
        localStorage.setItem("pw_notes", JSON.stringify(filtered));
      }
    } else {
      localStorage.setItem("pw_notes", JSON.stringify(filtered));
    }
  ...
  async function clearAllNotes() {
    ...
    if (!ok)
      return;
    localStorage.removeItem("pw_notes");
    localStorage.removeItem("pw_notes_enc");
  ...
  exportBtn.addEventListener("click", async () => {
    const notes = await Promise.resolve(loadNotes());
    if (!notes || !notes.length)
      return alert("No notes to export");
    if (encryptToggle.checked) {
      const pass = encPass.value;
      if (!pass)
        return alert("Enter passphrase");
      try {
        const payload = await encryptJSON(notes, pass);
        const wrapper = { exportedAt: (/* @__PURE__ */ new Date()).toISOString(), encrypted: true, payload };
        const blob = new Blob([JSON.stringify(wrapper, null, 2)], { type: "application/json" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = "password-notes-encrypted.json";
        document.body.appendChild(a);
        a.click();
        a.remove();
        URL.revokeObjectURL(url);
      } catch (e) {
        return alert("Encryption failed");
      }
    } else {
      const wrapper = { exportedAt: (/* @__PURE__ */ new Date()).toISOString(), encrypted: false, notes };
      const blob = new Blob([JSON.stringify(wrapper, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "password-notes.json";
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
  ...
  importFile.addEventListener("change", async (e) => {
    ...
    try {
      ...
      if (obj && obj.encrypted && obj.payload) {
        ...
        if (!pass)
          return alert("Passphrase required");
  ...
  async function handleImportMerge(incoming) {
    ...
    if (choice) {
      ...
      for (const n of incoming.concat(existing))
        map.set(n.name + "|" + n.pw, n);
  ...
  tabNotes.addEventListener("click", () => {
    ...
    tabNotes.setAttribute("aria-pressed", "true");
    tabHistory.setAttribute("aria-pressed", "false");
  });
  tabHistory.addEventListener("click", () => {
    notesView.hidden = true;
    historyView.hidden = false;
    tabNotes.setAttribute("aria-pressed", "false");
  ...
  unlockBtn.addEventListener("click", async () => {
    const pass = sessionPassInput.value;
    if (!pass)
      return alert("Enter session passphrase");
    const encRaw = localStorage.getItem("pw_notes_enc");
    if (encRaw) {
      try {
        const payload = JSON.parse(encRaw);
        await decryptJSON(payload, pass);
      } catch (e) {
        return alert("Unlock failed: wrong passphrase");
      }
    }
    ...
    if (persistEnc.checked) {
      try {
        ...
        const payload = await encryptJSON(notes, pass);
        localStorage.setItem("pw_notes_enc", JSON.stringify(payload));
        localStorage.setItem("pw_master_pass_hash", btoa(pass));
      } catch (e) {
        console.warn("persist encryption failed", e);
      }
    }
    unlockBtn.hidden = true;
    lockBtn.hidden = false;
    sessionPassInput.disabled = true;
    renderNotes();
    renderHistory();
  });
  lockBtn.addEventListener("click", () => {
    sessionStorage.removeItem("pw_session_pass");
    unlockBtn.hidden = false;
    lockBtn.hidden = true;
    sessionPassInput.disabled = false;
    sessionPassInput.value = "";
    renderNotes();
  });
